...
def infer_signal_type(url: str, snippet: str, *, query_id: str = "") -> str:
    ...
    if "direct message workflow signal" in text or any(
        x in text for x in ("whatsapp", "telegram", "messenger", "via dm")
    ):
        return "DIRECT_MESSAGE_WORKFLOW_SIGNAL"

    # Vendor cues: only when a known vendor/tool is actually present.
    if any(v in text for v in VENDOR_KEYWORDS) or any(
        v in url_lower for v in ("zendesk", "intercom", "greenhouse", "lever")
    ):
        return "VENDOR_CUE"
    ...
    if any(k in path for k in ("/team", "/leadership", "/people", "/about", "/org-chart", "/org")):
        if any(k in text for k in ORG_KEYWORDS) or any(
            x in text
            for x in (
                "director",
                "manager",
    ...
    has_term = any(k in text for k in PROCESS_KEYWORDS) or any(
        k in path
        for k in ("/billing", "/refund", "/procurement", "/onboarding", "/support", "/help", "/booking", "/reservation")
    )
    strong_phrase = any(
        p in text
        for p in (
...
def signal_counts(evidence_items: list[dict[str, Any]]) -> dict[str, int]:
    ...
    for ev in evidence_items or []:
        ...
        if not st:
            st = infer_signal_type(
                str(ev.get("url", "")), str(ev.get("snippet", "")), query_id=str(ev.get("query_id", ""))
            )
...
def compute_hypothesis_confidence(
    ...
    *,
    ...
    # Caps
    channel_only = bool(
        (has_contact or has_social) and not (has_process or has_vendor or has_org) and sum(counts.values()) > 0
    )
    ...
    if distinct_urls:
        policy_only = all(
            any(x in u for x in ("/privacy", "/terms", "/cookie", "/polic", "/gdpr")) for u in distinct_urls
        )
